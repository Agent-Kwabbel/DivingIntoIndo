        ...
        <section className="w-full pt-10">
            <div className="flex h-full w-full flex-col items-center justify-center">
                ...
                {status && statusOptions[status]?.description && (
                    <p className="text-sm text-primary pt-1 italic text-center px-4">
    ...
    return (
        <span className="relative flex h-4 w-4">
            {statusOptions[status].ping && (
                <span
                    className={`absolute animate-ping inline-flex h-full w-full rounded-full bg-${statusOptions[status].color}`}
                ></span>
            )}
            <span
                className={`relative inline-flex rounded-full h-4 w-4 bg-${statusOptions[status].color}`}
                title="Is the status circle pulsing? Then the trip is ongoing!"
            ></span>
        </span>
    );
}
